...
D_a = st.number_input("Number of Type A deliveries (0-2 kg)", min_value=0, value=20)
D_b = st.number_input("Number of Type B deliveries (2-10 kg)", min_value=0, value=10)
D_c = st.number_input("Number of Type C deliveries (10-200 kg)", min_value=0, value=5)
...
2. The sheet to be analyzed must contain columns named "Name", "Latitude", "Longitude", and "Weight (KG)".
...
if uploaded_file:
    ...
    if st.button("Extract Deliveries from Excel"):
        D_a, D_b, D_c, df_shops = extract_deliveries_from_excel(uploaded_file, sheet_name)
        if D_a is not None:
...
# Function for nearest neighbor route optimization
...
if uploaded_file:
    df_shops = pd.read_excel(uploaded_file, sheet_name=sheet_name)
    ...
    for cluster_id in clusters:
        ...
        st.write(f"Cluster {cluster_id} Total Distance: {total_distance / 1000:.2f} kilometers")  # Scale down for display
